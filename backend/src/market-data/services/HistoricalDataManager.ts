...
export class HistoricalDataManager {
    ...
    constructor( private prisma: PrismaClient, private assetConfig: PoolConfig[], private pricesApi: string) {}
    ...
    private async backfillDataForAsset(assetName: string, dates: number[]) {
        const [minDate, maxDate] = this.calculateMinMaxDate(dates)
        const priceData: any = await this.fetchDataFromCoinGecko(minDate, maxDate, assetName)
        const poolConfig = this.assetConfig.find( p => p.assetName == assetName)
        if(!poolConfig) {
            throw Error("Invalid config please check")
        }
        for(const [timestamp, priceInUsd] of priceData) {

...
// src/WHYPEStrategySwap.sol
contract WHYPEStrategySwap is ReentrancyGuard, Ownable {
    ...
    function swapUSDTtoWHYPE(
        ...
        uint256 WHYPEReceived = hyperSwap.exactInputSingle(params);
        if (WHYPEReceived == 0) revert SwapFailed();

        emit WHYPEStrategyExecuted(
            user,
            amount,
            WHYPEReceived,
            block.timestamp
